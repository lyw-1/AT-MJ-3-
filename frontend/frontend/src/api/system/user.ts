...
export const getUserByUsername = (username: string) => {
  return request.get<UserResponse>(`/users/username/${username}`)
}
...
export const resetUserPassword = (id: number, password: string) => {
  return request.post(`/users/${id}/reset-password`, { newPassword: password })
}

// 修改用户密码（需要原密码验证）
export const changeUserPassword = (id: number, oldPassword: string, newPassword: string) => {
  return request.post(`/users/${id}/change-password`, { oldPassword, newPassword })
}

// 为用户批量分配角色
export const batchAssignRolesToUser = (userId: number, roleIds: number[]) => {
  return request.post(`/user-roles/batch-assign?userId=${userId}`, roleIds)
}
